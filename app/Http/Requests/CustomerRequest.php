...
class CustomerRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        if ($this->customer) {
            $rules['email'] = ['required', 'email', 'max:100',  Rule::unique('customers')->ignore($this->customer)];
            $rules['password'] = ['nullable', 'string', 'min:8', 'confirmed'];
            $rules['password_confirmation'] = ['nullable', 'string', 'min:8'];
        }
        if ($this->employee) {
            $rules['email'] = ['required', 'email', 'max:100',  Rule::unique('customers')->ignore($this->employee)];
            $rules['password'] = ['nullable', 'string', 'min:8', 'confirmed'];
            $rules['password_confirmation'] = ['nullable', 'string', 'min:8'];
        }
        return $rules;
